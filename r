...
[JTMresume.tex](JTMresume.tex) file to use fonts installed on your machine.

To build the resume on Linux, simply use make from the command line.A

The resume generator will utilize the [references.bib](references.bib) file for
...
The reference file regeneration uses a custom script, [extract-by-tags.py](extract-by-tags.py) to
...
dynamic content is located in the [sections/](sections/) directory.  The
[resume.tex](resume.tex) file in the main directory is the master LaTeX file that
references these files and the style class, [JTMresume.sty](JTMresume.sty).
